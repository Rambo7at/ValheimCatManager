...
public class ResManager
{
    ...
    public void AddItem(string itemName, bool mockCheck)
    {
        ...
        if (ab.LoadAsset<GameObject>(itemName) is not GameObject itemPrefab)
        {
            Debug.LogWarning($"[ResManager.AddItem] 未找到预制件 [{itemName}]，已跳过");
            return;
        }
        ...
        if (!mockCheck) return;
        if (!MockSystem.Instance.mockPrefabDict.ContainsKey(hash)) MockSystem.Instance.mockPrefabDict.Add(hash, itemPrefab.name);
    }



    /// <summary>注：加载装备预制件并同时创建配方，一次性注册装备和制作配方</summary>
    public void AddEquipment(string itemName, bool mockCheck, string craftingStationName, int minStationLevel, int amount, params (string resItem, int resAmount, int levelAmount, bool upgraderRes)[] resItemList)
    {
        var modName = Assembly.GetCallingAssembly().GetName().Name;
        ...
        if (!mockCheck) return;
        if (!MockSystem.Instance.mockPrefabDict.ContainsKey(hash)) MockSystem.Instance.mockPrefabDict.Add(hash, itemPrefab.name);
        AddRecipe(itemName, craftingStationName, minStationLevel, amount, resItemList);
    }

    /// <summary>注：快速创建配方配置并注册到配方管理器</summary>
    public void AddRecipe(string itemName, string craftingStationName, int minStationLevel, int amount, params (string resItem, int resAmount, int levelAmount)[] resItemList)
    {
        if (string.IsNullOrEmpty(itemName))
        {
            Debug.LogError($"[ResManager.AddRecipe] 配方物品名为空，已跳过");
            return;
        }

        RecipeConfig recipeConfig = new RecipeConfig(itemName, craftingStationName, minStationLevel, amount, resItemList);

        RecipeManager.Instance.AddRecipe(recipeConfig);
    }

    /// <summary>注：重载方法，快速创建带升级专用材料的配方配置并注册到配方管理器</summary>
    public void AddRecipe(string itemName, string craftingStationName, int minStationLevel, int amount, params (string resItem, int resAmount, int levelAmount, bool upgraderRes)[] resItemList)
    {
        if (string.IsNullOrEmpty(itemName))
        {
            Debug.LogError($"[ResManager.AddRecipe] 配方物品名为空，已跳过");
    ...
    public void AddVegetation(VegetationConfig vegetationConfig, bool mock)
    {
        ...
        VegetationManager.Instance.VegetationList.Add(vegetationConfig);
    ...
    public void AddFood(string foodName, bool mockCheck)
    {
        var modName = Assembly.GetCallingAssembly().GetName().Name;
        if (GetAssetBundle(modName) is not AssetBundle ab) return;
        ...
        pieceConfig.目录 = Piece.UsageTagFlags.Food;
        pieceConfig.制作工具 = PieceManager.E_CraftTool.Feaster;
