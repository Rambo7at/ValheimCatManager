...
public class PrefabManager
{
    ...
    /// <summary> 注：物品字典，准备注册给游戏 </summary>
    public readonly Dictionary<int, GameObject> customItemDict = new();
    /// <summary>注：预制件字典，准备注册给游戏 </summary>
    public readonly Dictionary<int, GameObject> customPrefabDict = new();
    /// <summary>注：地点预制件字典，准备注册给游戏</summary>
    public readonly Dictionary<int, GameObject> customLocationDict = new();



    private void RegisterItemPatch(ObjectDB objectDB)
    {
        if (SceneManager.GetActiveScene().name == "main") Instance.RegisterToObjectDB(objectDB, Instance.customItemDict);
    }

    private void RegisterGameObjectPatch(ZNetScene zNetScene)
    {
        if (SceneManager.GetActiveScene().name == "main")
        {
            Instance.RegisterToZNetScene(zNetScene, Instance.customItemDict);
            Instance.RegisterToZNetScene(zNetScene, Instance.customPrefabDict);
            Instance.RegisterLocationToZNetScene(zNetScene, Instance.customLocationDict);
        }
    }

    private void CopyOtherDBPatch(ObjectDB objectDB, ObjectDB other)
    {
