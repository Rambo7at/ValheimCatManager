...
namespace ValheimCatManager.Managers
{
    public class CookingStationManager
    {
        ...
        public void RegisterCookingStationConfig(List<CookingStationConfig> cookingStationConfigs)
        {
            ...
            {
                // 加载烹饪站、输入物品、输出物品的预制件
                GameObject prefabCookingStation = CatTool.GetGameObject(item.PrefabName);
                GameObject prefabInputItem = CatTool.GetGameObject(item.InputItem);
                GameObject prefabOutputItem = CatTool.GetGameObject(item.OutputItem);

                // 任一预制件为空时打印错误并跳过
                if (prefabCookingStation == null || prefabInputItem == null || prefabOutputItem == null)
                {
                    Debug.LogError($"执行RegisterCookingStationConfig时，有单个预制件是空[{item.PrefabName}],[{item.InputItem}],[{item.OutputItem}]，已跳过");
                    continue;
                }
                ...
                {
                    Debug.LogError($"执行RegisterCookingStationConfig时，[{item.PrefabName}] 没有对应的[CookingStation组件]，已跳过");
                    continue;
                }
                ...
                {
                    Debug.LogError($"执行RegisterCookingStationConfig时，物品：{item.InputItem}或{item.OutputItem} 没有[ItemDrop组件]，已跳过");
                    continue;
                }
